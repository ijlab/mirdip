...
def standardize_normalize_integrate_bitargeting_run(predictions,
                                                    save_cleaned_results_to,
                                                    names=['ignore1', 'num_sites', 'bitargeting_score', 'mirdip4_mirbase_id', 'ensembl_gene_id'],
                                                    gene_colname='ensembl_gene_id',
                                                    mir_colname='mirdip4_mirbase_id',
                                                    score_colname='bitargeting_score',
                                                    data_source='bitargeting_May_2021',
                                                    group=2):
        bitargeting = pd.read_csv(predictions, sep='\t', header=None, names=names)
        bitargeting = bitargeting.astype({score_colname:'float64'})
        bitargeting = bitargeting[~(bitargeting[gene_colname].isin(utrs_to_exclude))]
        bitargeting[gene_colname] = bitargeting[gene_colname].str.split('|').apply(lambda x: x[0])
        bitargeting = normalize_scores(bitargeting, score_colname, group=group)
        bitargeting['original_gene_symbol'] = ''
        bitargeting['original_mirbase_id'] = ''
        bitargeting['data_source'] = data_source
        bitargeting = map_genes_with_hgnc(bitargeting, 'ensembl_gene_id', 'ensembl_gene_id', check_ensembl=True, hgnc=None)
        bitargeting = bitargeting[['symbol', 'mirdip4_mirbase_id', 'bitargeting_score', 'bitargeting_score_norm', 'data_source', 'original_gene_symbol', 'original_mirbase_id']]
        bitargeting.replace(r'^\s*$', np.nan, regex=True)
        bitargeting.dropna(inplace=True)
        bitargeting = bitargeting.drop_duplicates()
        bitargeting = group_mir_gene_pairs_and_take_ranked_product(bitargeting, 'bitargeting_score_norm')
        bitargeting = normalize_scores(bitargeting, score_colname+'_norm', normalized_score_column_name=score_colname+'_norm', group=2)
        bitargeting.sort_values(by='bitargeting_score_norm', inplace=True, ascending=True)
        bitargeting['original_gene_symbol'] = bitargeting['symbol']
        bitargeting['original_mirbase_id'] = bitargeting['mirdip4_mirbase_id']
        bitargeting.to_csv(save_cleaned_results_to, sep='\t',index=False,header=False, quoting=csv.QUOTE_MINIMAL)

standardize_normalize_integrate_bitargeting_run('/home/waddelld/rnatools/bitargeting/bitargeting_known_for_mirdip5_tmp.tsv',
                                                        '/home/waddelld/rnatools/mirdip5/resources_redo_final/bitargeting_may_2021.txt')
