...
# Load mirnatip
mirnatip = pd.read_csv('/home/waddelld/rnatools/hsa_all_predictions.txt', sep=',', header=None, names=['ensembl_gene_id', 'mirdip4_mirbase_id', 'mirnatip_score'])
# check for corruption, all the %>$() characters we saw before with the tar.gz archive - should have been a zip, was on redownload
mirnatip = mirnatip.astype({'mirnatip_score':'float64'})
mirnatip = normalize_scores(mirnatip, 'mirnatip_score', group=2)
mirnatip['original_gene_symbol'] = '' 
mirnatip['original_mirbase_id'] = ''
mirnatip['data_source'] = 'MiRNATIP'
mirnatip = normalize_scores(mirnatip, 'mirnatip_score', group=2)
mirnatip = map_genes_with_hgnc(mirnatip, 'ensembl_gene_id', 'ensembl_gene_id', check_ensembl=True, hgnc=None)
mirnatip = mirnatip[['symbol', 'mirdip4_mirbase_id', 'mirnatip_score', 'mirnatip_score_norm', 'data_source', 'original_gene_symbol', 'original_mirbase_id']]
mirnatip.replace(r'^\s*$', np.nan, regex=True)
mirnatip.dropna(inplace=True)
mirnatip = mirnatip.drop_duplicates()
mirnatip = group_mir_gene_pairs_and_take_ranked_product(mirnatip, 'mirnatip_score_norm')
mirnatip = normalize_scores(mirnatip, 'mirnatip_score_norm', normalized_score_column_name='mirnatip_score_norm', group=2)
mirnatip.sort_values(by='mirnatip_score_norm', inplace=True, ascending=True)
mirnatip['original_gene_symbol'] = mirnatip['symbol']
mirnatip['original_mirbase_id'] = mirnatip['mirdip4_mirbase_id']
mirnatip = mirnatip.drop_duplicates()
mirnatip.to_csv('/home/waddelld/rnatools/mirdip5/resources_redo_final/mirnatip.txt', sep='\t',index=False,header=False, quoting=csv.QUOTE_MINIMAL)
