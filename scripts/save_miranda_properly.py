...
def standardize_normalize_integrate_miranda_run(predictions, 
                                             save_cleaned_results_to,
                                             names=['mirdip4_mirbase_id', 'ensembl_gene_id', 'miranda_score',
                                                    'miranda_energy', 'miranda_int_score_best', 'miranda_energy_best',
                                                    'ignore1', 'ignore2', 'ignore3', 'ignore4'],
                                             gene_colname='ensembl_gene_id',
                                             mir_colname='mirdip4_mirbase_id',
                                             score_colname='miranda_score',
                                             data_source='miranda_May_2021',
                                             group=1):
    miranda = pd.read_csv(predictions, sep='\t', header=None, names=names)
    miranda.dropna(inplace=True)
    miranda = miranda.astype({gene_colname:'str', mir_colname:'str', score_colname:'float64'})
    miranda = miranda[~(miranda[gene_colname].isin(utrs_to_exclude))]
    miranda[gene_colname] = miranda[gene_colname].str.split('|').apply(lambda x: x[0])
    miranda = normalize_scores(miranda, score_colname, group=group)
    miranda['original_gene_symbol'] = ''
    miranda['original_mirbase_id'] = ''
    miranda['data_source'] = data_source
    miranda = map_genes_with_hgnc(miranda, 'ensembl_gene_id', 'ensembl_gene_id', check_ensembl=True, hgnc=None)
    miranda = miranda[['symbol', 'mirdip4_mirbase_id', 'miranda_score', 'miranda_score_norm', 'data_source', 'original_gene_symbol', 'original_mirbase_id']]
    miranda.replace(r'^\s*$', np.nan, regex=True)
    miranda.dropna(inplace=True)
    miranda = miranda.drop_duplicates()
    miranda = group_mir_gene_pairs_and_take_ranked_product(miranda, 'miranda_score_norm')
    miranda = normalize_scores(miranda, score_colname+'_norm', normalized_score_column_name=score_colname+'_norm', group=2)
    miranda.sort_values(by='miranda_score_norm', inplace=True, ascending=True)
    miranda['original_gene_symbol'] = miranda['symbol']
    miranda['original_mirbase_id'] = miranda['mirdip4_mirbase_id']
    miranda.to_csv(save_cleaned_results_to, sep='\t',index=False,header=False, quoting=csv.QUOTE_MINIMAL)
    
standardize_normalize_integrate_miranda_run('/home/waddelld/rnatools/miranda/miRanda-3.3a/miranda_known.scans',
                                     '/home/waddelld/rnatools/mirdip5/resources_redo_final/miranda_may_2021.txt')
