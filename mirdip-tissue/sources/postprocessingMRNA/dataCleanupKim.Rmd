---
title: "Processing Kim gene expression data"
...
meta$Sample_ID <- gsub("BioSample: https://www.ncbi.nlm.nih.gov/biosample/", "", meta$X.Sample_relation.1) # extract only SAMN IDs from X.Sample_relation
```
...
```{r}
meta$Tissue <- "lung"
meta$Disease <- ifelse(meta$X.Sample_characteristics_ch1.2 == "tissue type: tumor", "adenocarcinoma", "normal")
meta$Group <- apply(meta[,c(ncol(meta)-1,ncol(meta))], 1, function(x) paste(x[1], x[2], sep="_"))
meta$Group <- str_replace_all(meta$Group, "_normal", "")
meta$Disease <- str_replace_all(meta$Disease, "normal", "")
head(meta)
```
...
# Rename the columns in countsFile with SampleID. (Currently the column names are the run_accession)
...
```{r}
write.table(data_long_final, "./kim_longData.txt", sep = "\t", row.names = F, col.names = T, quote = F)
write.table(unique(data_long_final[,1]), "./kim_genes.txt", sep = "\t", row.names = F, col.names = T, quote = F)
writeLines(as.character(unique(data_long_final$Tissue)), "./kim_tissues.txt")
```
