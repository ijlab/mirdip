---
title: "Processing Mao gene expression data"
...
```{r}
meta$Tissue <- str_replace_all(meta$X.Sample_characteristics_ch1, "tissue: ", "")
meta$Tissue <- tolower(meta$Tissue)
meta$Tissue <- str_replace_all(meta$Tissue, " ", ".")
meta$Disease <- str_replace_all(meta$X.Sample_characteristics_ch1.1, "disease state: ", "")
...
```{r}
write.table(data_long_final, "./mao_longData.txt", sep = "\t", row.names = F, col.names = T, quote = F)
write.table(unique(data_long_final[,1]), "./mao_genes.txt", sep = "\t", row.names = F, col.names = T, quote = F)
writeLines(as.character(unique(data_long_final$Tissue)), "./mao_tissues.txt")
```
