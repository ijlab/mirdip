---
title: "Processing Lyu gene expression data"
...
```{r}
meta$Tissue <- tolower(meta$X.Sample_characteristics_ch1.1)
meta$Tissue <- str_replace_all(meta$Tissue, "organ: ", "")
...
meta$Group <- str_replace_all(meta$Group, "_adjecent.normal", "") # adj normal means no disease
meta$Disease <- str_replace_all(meta$Disease, "adjecent normal", "") # no disease
head(meta)
```
...
```{r}
write.table(data_long_final, "./lyu_longData.txt", sep = "\t", row.names = F, col.names = T, quote = F)
write.table(unique(data_long_final[,1]), "./lyu_genes.txt", sep = "\t", row.names = F, col.names = T, quote = F)
writeLines(as.character(unique(data_long_final$Tissue)), "./lyu_tissues.txt")
```
