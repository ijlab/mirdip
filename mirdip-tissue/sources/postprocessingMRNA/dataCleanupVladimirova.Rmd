---
title: "Processing Vladimirova gene expression data"
...
library(reshape2)
setwd("/data/RNA/vladimirovaData/")
```
...
#normalizedData <- apply(countsFile[,-c(1)], 2, quintize)
#normalizedData <- data.frame(normalizedData)
#normalizedData <- cbind(countsFile[,1], normalizedData)
#names(normalizedData)[names(normalizedData) == 'countsFile[, 1]'] <- 'Gene'
...
```{r}
meta$Tissue <- str_replace_all(meta$X.Sample_source_name_ch1, ", primary tumor", "")
meta$Tissue <- str_replace_all(meta$Tissue, " ", ".")
meta[96:97,]$Tissue <- str_replace_all(meta[96:97,]$Tissue, "Thyroid.tumor", "Thyroid.gland")
meta$Tissue <- tolower(meta$Tissue)
meta$Disease <- str_replace(meta$X.Sample_characteristics_ch1, "subtype: ", "")
meta$Disease <- str_replace_all(meta$Disease, " ", ".")
meta[96:97,]$Disease <- str_replace_all(meta[96:97,]$Disease, "disease:.Thyroid.tumor", "thyroid.cancer")
...
# Rename the columns in normalizedData with SampleID. (Currently the column names are run_accession)
...
```{r}
write.table(data_long_final, "./vladimirova_longData.txt", sep = "\t", row.names = F, col.names = T, quote = F)
write.table(unique(data_long_final[,1]), "./vladimirova_genes.txt", sep = "\t", row.names = F, col.names = T, quote = F)
writeLines(as.character(unique(data_long_final$Tissue)), "./vladimirova_tissues.txt")
```
