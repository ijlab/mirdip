---
title: "Processing GE gene expression data"
...
```{r}
#countsFile <- data.frame(countsFile)
...
combinedData <- lapply(uniqueTissues, function(x) {
  columnsToFetch <- meta$Sample_ID[which(meta$Group == x)]
...
```{r}
write.table(data_long_final, "./ge_longData.txt", sep = "\t", row.names = F, col.names = T, quote = F)
write.table(unique(data_long_final[,1]), "./ge_genes.txt", sep = "\t", row.names = F, col.names = T, quote = F)
writeLines(as.character(unique(data_long_final$Tissue)), "./ge_tissues.txt")
```
