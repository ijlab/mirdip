---
title: "Processing Ge expression data"
...
knitr::opts_knit$set(root.dir = paste(sep="", root_path, "dat/miRNA/geData/"))
setwd(paste(sep="", root_path, "data/miRNA/geData/"))
```
...
# remove SAMN10319858 because the FASTQ file parsing failed and therefore it was removed from the samplesheet.
if ("SAMN10319858" %in% meta$Sample_ID) {
  meta <- meta[-which(meta$Sample_ID == "SAMN10319858"),] 
}
meta
```
...
meta$Disease <- str_replace_all(meta$X.Sample_source_name_ch1, " tissue", "")
meta$Disease <- str_replace_all(meta$Disease, " ", ".")
meta$Group <- apply(meta[,c(ncol(meta)-1,ncol(meta))], 1, function(x) paste(x[1], x[2], sep="_"))
meta$Disease <- str_replace_all(meta$Disease, "colorectal.pericarcinomatous", "") #pericarcinomatous means around a carcinoma, so I define it as normal
meta$Group <- str_replace_all(meta$Group, "_colorectal.pericarcinomatous", "")
head(meta)
```
...
```{r}
write.table(data_long, paste(sep="", result_path,"/ge_longData.txt"), sep = "\t", row.names = F, col.names = T, quote = F)
write.table(unique(data_long[,1:3]), paste(sep="", result_path,"/ge_miRNAs.txt"), sep = "\t", row.names = F, col.names = T, quote = F)
writeLines(as.character(unique(data_long$Tissue)), paste(sep="", result_path,"/ge_tissues.txt"))
```