---
title: "Processing Hua expression data"
...
knitr::opts_knit$set(root.dir = paste(sep="", root_path, "data/miRNA/huaData/"))
setwd(paste(sep="", root_path, "data/miRNA/huaData/"))
```
...
meta$Disease <- tolower(meta$X.Sample_source_name_ch1)
meta$Disease <- str_replace_all(meta$Disease, " ", ".")
meta$Group <- apply(meta[,c(ncol(meta)-1,ncol(meta))], 1, function(x) paste(x[1], x[2], sep="_"))
meta$Group <- str_replace_all(meta$Group, "_healthy.donor", "")
meta$Disease <- str_replace_all(meta$Disease, "healthy.donor", "")
head(meta)
```
...
```{r}
write.table(data_long, paste(sep="", result_path,"/hua_longData.txt"), sep = "\t", row.names = F, col.names = T, quote = F)
write.table(unique(data_long[,1:3]),paste(sep="", result_path,"/hua_miRNAs.txt"), sep = "\t", row.names = F, col.names = T, quote = F)
writeLines(as.character(unique(data_long$Tissue)),paste(sep="", result_path,"/hua_tissues.txt"))
```
