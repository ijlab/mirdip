---
title: "Processing Vladimirova expression data"
...
knitr::opts_knit$set(root.dir = paste(sep="", root_path, "data/miRNA/vladimirovaData/"))
setwd(paste(sep="", root_path, "data/miRNA/vladimirovaData"))
```
...
```{r}
write.table(data_long, paste(sep="", result_path,"/vladimirova_longData.txt"), sep = "\t", row.names = F, col.names = T, quote = F)
write.table(unique(data_long[,1:3]), paste(sep="", result_path,"/vladimirova_miRNAs.txt"), sep = "\t", row.names = F, col.names = T, quote = F)
writeLines(as.character(unique(data_long$Tissue)), paste(sep="", result_path,"/vladimirova_tissues.txt"))
```