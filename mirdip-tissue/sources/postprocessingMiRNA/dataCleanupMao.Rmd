---
title: "Processing Mao expression data"
...
knitr::opts_knit$set(root.dir = paste(sep="", root_path, "data/miRNA/maoData/"))
setwd(paste(sep="", root_path, "data/miRNA/maoData/"))
...
meta <- meta[(meta$X.Sample_characteristics_ch1.2 == "molecule subtype: miRNA"),] #remove non-miRNAs
...
```{r}
meta$Tissue <- str_replace(meta$X.Sample_characteristics_ch1.1, "tissue: ", "")
meta$Tissue <- str_replace_all(meta$Tissue, " ", ".")
meta$Tissue <- tolower(meta$Tissue)
meta$Disease <- str_replace_all(meta$X.Sample_characteristics_ch1, "subject status: ", "")
...
meta$Disease <- str_replace_all(meta$Disease, "normal.control", "")
meta$Group <- str_replace_all(meta$Group, "_normal.control", "")
head(meta)
```
...
```{r}
write.table(data_long, paste(sep="", result_path,"/mao_longData.txt"), sep = "\t", row.names = F, col.names = T, quote = F)
write.table(unique(data_long[,1:3]), paste(sep="", result_path,"/mao_miRNAs.txt"), sep = "\t", row.names = F, col.names = T, quote = F)
writeLines(as.character(unique(data_long$Tissue)), paste(sep="", result_path,"/mao_tissues.txt"))
```