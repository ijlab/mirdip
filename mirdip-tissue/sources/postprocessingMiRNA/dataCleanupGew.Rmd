---
title: "Processing GEW expression data"
...
knitr::opts_knit$set(root.dir = paste(sep="", root_path, "data/miRNA/geWData/"))
setwd(paste(sep="", root_path, "data/miRNA/geWData/"))
...
```{r}
meta$Tissue <- "testis"
meta$Disease <- str_replace_all(meta$X.Sample_source_name_ch1, " ", ".")
meta$Group <- apply(meta[,c(ncol(meta)-1,ncol(meta))], 1, function(x) paste(x[1], x[2], sep="_"))
meta$Group <- str_replace_all(meta$Group, "_normal.control.testis", "") # normal means no disease
meta$Disease <- str_replace_all(meta$Disease, "normal.control.testis", "") # no disease
head(meta)
```
...
```{r}
write.table(data_long, paste(sep="", result_path,"/geW_longData.txt"), sep = "\t", row.names = F, col.names = T, quote = F)
write.table(unique(data_long[,1:3]), paste(sep="", result_path,"/geW_miRNAs.txt"), sep = "\t", row.names = F, col.names = T, quote = F)
writeLines(as.character(unique(data_long$Tissue)), paste(sep="", result_path,"/geW_tissues.txt"))
```