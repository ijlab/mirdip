...
# Load rna22
rna22 = pd.read_csv('/home/waddelld/rnatools/mirdip5/rna22.txt', sep='\t', header=0, names=['ensembl_gene_id', 'mirdip4_mirbase_id', 'rna22_score'])
# check for corruption, all the %>$() characters we saw before with the tar.gz archive - should have been a zip, was on redownload
rna22 = rna22.astype({'rna22_score':'float64'})
rna22 = normalize_scores(rna22, 'rna22_score', group=2)
rna22['original_gene_symbol'] = '' 
rna22['original_mirbase_id'] = ''
rna22['data_source'] = 'RNA22'
rna22 = normalize_scores(rna22, 'rna22_score', group=2)
rna22 = map_genes_with_hgnc(rna22, 'ensembl_gene_id', 'ensembl_gene_id', check_ensembl=True, hgnc=None)
rna22 = rna22[['symbol', 'mirdip4_mirbase_id', 'rna22_score', 'rna22_score_norm', 'data_source', 'original_gene_symbol', 'original_mirbase_id']]
rna22.replace(r'^\s*$', np.nan, regex=True)
rna22.dropna(inplace=True)
rna22 = rna22.drop_duplicates()
rna22 = group_mir_gene_pairs_and_take_ranked_product(rna22, 'rna22_score_norm')
rna22 = normalize_scores(rna22, 'rna22_score_norm', normalized_score_column_name='rna22_score_norm', group=2)
rna22.sort_values(by='rna22_score_norm', inplace=True, ascending=True)
rna22['original_gene_symbol'] = rna22['symbol']
rna22['original_mirbase_id'] = rna22['mirdip4_mirbase_id']
rna22 = rna22.drop_duplicates()
rna22.to_csv('/home/waddelld/rnatools/mirdip5/resources_redo_final/rna22.txt', sep='\t',index=False,header=False, quoting=csv.QUOTE_MINIMAL)
