...
def standardize_normalize_integrate_mirmap_run(predictions,
                                               save_cleaned_results_to,
                                               names=['mirdip4_mirbase_id', 'ensembl_gene_id', 'end_sites', 'seed_lengths', 'nb_mismatches_except_gu_wobbles', 'nb_gu_wobbles', 'tgs_aus', 'tgs_positions', 'tgs_pairing3ps', 'tgs_scores', 'dg_duplexs', 'dg_bindings', 'dg_duplex_seeds', 'dg_binding_seeds', 'dg_opens', 'dg_totals', 'prob_exacts', 'prob_binomials', 'cons_blss', 'selec_phylops', 'mirmap_score'],
                                               gene_colname='ensembl_gene_id',
                                               mir_colname='mirdip4_mirbase_id',
                                               score_colname='mirmap_score',
                                               data_source='mirmap_May_2021',
                                               excluded_utrs=utrs_to_exclude,
                                               group=1):
    mirmap = pd.read_csv(predictions, sep='\t', header=None, names=names)
    mirmap = mirmap[[gene_colname, mir_colname, score_colname]]
    mirmap = mirmap.astype({score_colname:'float64'})
    mirmap = mirmap[~(mirmap[gene_colname].isin(utrs_to_exclude))]
    mirmap[gene_colname] = mirmap[gene_colname].str.split('|').apply(lambda x: x[0])
    mirmap[mir_colname] = mirmap[mir_colname].str.split(' ').apply(lambda x: x[0])
    mirmap = normalize_scores(mirmap, score_colname, group=group)
    mirmap['original_gene_symbol'] = ''
    mirmap['original_mirbase_id'] = ''
    mirmap['data_source'] = data_source
    mirmap = map_genes_with_hgnc(mirmap, 'ensembl_gene_id', 'ensembl_gene_id', check_ensembl=True, hgnc=None)
    mirmap = mirmap[['symbol', 'mirdip4_mirbase_id', 'mirmap_score', 'mirmap_score_norm', 'data_source', 'original_gene_symbol', 'original_mirbase_id']]
    mirmap.replace(r'^\s*$', np.nan, regex=True)
    mirmap.dropna(inplace=True)
    mirmap = mirmap.drop_duplicates()
    mirmap = group_mir_gene_pairs_and_take_ranked_product(mirmap, 'mirmap_score_norm')
    mirmap = normalize_scores(mirmap, score_colname+'_norm', normalized_score_column_name=score_colname+'_norm', group=2)
    mirmap.sort_values(by='mirmap_score_norm', inplace=True, ascending=True)
    mirmap['original_gene_symbol'] = mirmap['symbol']
    mirmap['original_mirbase_id'] = mirmap['mirdip4_mirbase_id']
    mirmap.to_csv(save_cleaned_results_to, sep='\t',index=False,header=False, quoting=csv.QUOTE_MINIMAL)

standardize_normalize_integrate_mirmap_run('/home/waddelld/rnatools/mirmap/mirmap_known.tsv',
                                           '/home/waddelld/rnatools/mirdip5/resources_redo_final/mirmap_may_2021.txt')
