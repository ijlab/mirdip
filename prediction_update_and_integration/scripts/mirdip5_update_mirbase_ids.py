...
doc = """Usage: update_data_source_ids.py --header --sep SEP --path PATH --mircol MIRCOL --genecol GENECOL --deadext DEADEXT --changedext CHANGEDEXT
...
def update_mirbase_ids(options):
    ...
    df = pd.read_csv(df_path, sep='\t', header=None, quoting=csv.QUOTE_NONE, names=['symbol', 'mirdip4_mirbase_id', 'score', 'score_norm', 'data_source', 'original_gene_symbol', 'original_mirbase_id'])
...
if __name__ == "__main__":
    ...
    df = update_mirdip4_gene_symbols(df, arguments['--checker'], arguments['--genecol'] if arguments['--genecol'] else 'symbol')
