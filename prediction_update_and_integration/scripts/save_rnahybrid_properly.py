...
def standardize_normalize_integrate_rnahybrid_run(predictions,
                                                  save_cleaned_results_to,
                                                  names=['ensembl_gene_id', 'ignore1', 'mirdip4_mirbase_id', 'ignore2', 'rnahybrid_score', 'pvalue','ignore3', 'ignore4', 'ignore5', 'ignore6', 'ignore7', 'ignore8', 'ignore9'],
                                                  mir_colname='mirdip4_mirbase_id',
                                                  gene_colname='ensembl_gene_id',
                                                  score_colname='rnahybrid_score',
                                                  data_source='rnahybrid_May_2021',
                                                  utrs_to_exclude=utrs_to_exclude,
                                                  group=2):
        rnahybrid = pd.read_csv(predictions, sep='\t', header=None, names=names)
        rnahybrid = rnahybrid.astype({score_colname:'float64'})
        rnahybrid = rnahybrid[~(rnahybrid[gene_colname].isin(utrs_to_exclude))]
        rnahybrid[gene_colname] = rnahybrid[gene_colname].str.split('|').apply(lambda x: x[0])
        rnahybrid = normalize_scores(rnahybrid, score_colname, group=group)
        rnahybrid['original_gene_symbol'] = ''
        rnahybrid['original_mirbase_id'] = ''
        rnahybrid['data_source'] = data_source
        rnahybrid = map_genes_with_hgnc(rnahybrid, 'ensembl_gene_id', 'ensembl_gene_id', check_ensembl=True, hgnc=None)
        rnahybrid = rnahybrid[['symbol', 'mirdip4_mirbase_id', 'rnahybrid_score', 'rnahybrid_score_norm', 'data_source', 'original_gene_symbol', 'original_mirbase_id']]
        rnahybrid.replace(r'^\s*$', np.nan, regex=True)
        rnahybrid.dropna(inplace=True)
        rnahybrid = rnahybrid.drop_duplicates()
        rnahybrid = group_mir_gene_pairs_and_take_ranked_product(rnahybrid, 'rnahybrid_score_norm')
        rnahybrid = normalize_scores(rnahybrid, score_colname+'_norm', normalized_score_column_name=score_colname+'_norm', group=2)
        rnahybrid.sort_values(by='rnahybrid_score_norm', inplace=True, ascending=True)
        rnahybrid['original_gene_symbol'] = rnahybrid['symbol']
        rnahybrid['original_mirbase_id'] = rnahybrid['mirdip4_mirbase_id']
        rnahybrid.to_csv(save_cleaned_results_to, sep='\t',index=False,header=False, quoting=csv.QUOTE_MINIMAL)

standardize_normalize_integrate_rnahybrid_run('/home/waddelld/rnatools/rnahybrid/work_known/rnahybrid_known_filtered.tsv',
                                              '/home/waddelld/rnatools/mirdip5/resources_redo_final/rnahybrid_may_2021.txt')
